...
// 순수 텍스트 로직(추출·발음교정·정렬)은 같은 폴더의 text.ts에 둔다.
...
function postsDir(locale: SpeechLocale): string {
  return path.join(process.cwd(), "src", "content", "posts", locale);
}
...
async function generate(file: string, locale: SpeechLocale): Promise<void> {
  ...
  const finalTimingPath = path.join(outDir, "audio.json");
  const nextPath = path.join(outDir, ".audio.next.mp3");
  const nextTimingPath = path.join(outDir, ".audio.next.json");
  ...
  try {
    ...
    fs.rmSync(nextTimingPath, { force: true });

    if (parts.length === 1) {
      fs.renameSync(parts[0], nextPath);
    } else {
      fs.writeFileSync(
        listFile,
        parts.map((p) => `file '${path.resolve(p)}'`).join("\n"),
      );
      execFileSync(
        "ffmpeg",
        [
    ...
    fs.writeFileSync(nextTimingPath, JSON.stringify(timed));
    fs.renameSync(nextPath, finalPath);
    fs.renameSync(nextTimingPath, finalTimingPath);
...
const hasAudio = (slug: string, locale: SpeechLocale) =>
  hasAudioPair(outDirFor(slug, locale));

async function main() {
  const args = process.argv.slice(2);
  const all = args.includes("--all");
  // --locale=en / --en : 영어 본문(src/content/posts/en) → public/posts/<slug>/en/
  const localeOption = args.find((arg) => arg.startsWith("--locale="));
  const localeValue = localeOption?.slice("--locale=".length);
  if (localeValue && localeValue !== "ko" && localeValue !== "en") {
    throw new Error(`unsupported locale: ${localeValue}`);
  }
  if (args.includes("--en") && localeValue === "ko") {
    throw new Error("conflicting locale options: --en and --locale=ko");
  }
  const locale: SpeechLocale = args.includes("--en")
    ? "en"
    : localeValue === "en"
      ? "en"
      : "ko";
  ...
  if (slug) {
    ...
    if (files.length === 0) {
      console.log(
        "오디오 없는 글이 없습니다. 전체를 다시 만들려면 --all 을 붙이세요.",
      );
      return;
    }
  }
  console.log(
    `locale=${locale}  voice=${VOICES[locale]}  posts=${files.length}`,
  );
